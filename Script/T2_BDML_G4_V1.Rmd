...
##2.6 Resultados Logit 
```{r}
summary(mod_1_logit,type="text")
```
## 2.7 Probabilidad Logit
...
## 2.11 Grafico de Resultados 

```{r}
## Mosaico por regiones
...
predictSample_EN <- test_factors   %>% 
  mutate(pobre_lab = predict(mod_1_EN, newdata = test_factors, type = "raw"))%>% 
  select(id,pobre_lab)
...
predict_logit <- data.frame(
  ...
  prob_hat = predict(prob_hat, newdata = credit, type = "prob"),     ## predicted class probabilities
