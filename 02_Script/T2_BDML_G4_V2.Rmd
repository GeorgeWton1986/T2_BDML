...
##1.6 Consolidar las BD Train, Test Hogares y estadísticas descriptivas
...
estadisticas_train <- skim(train)
estadisticas_test <- skim(test)

# Guardar 
write.table(estadisticas_train, "../04_Views/TABLA1-ESTADISTICAS_TRAIN.txt", sep = "\t", row.names = FALSE, quote = FALSE)
write.table(estadisticas_test, "../04_Views/TABLA2-ESTADISTICAS_TEST.txt", sep = "\t", row.names = FALSE, quote = FALSE)

# Selecciona variables clave para modelos
vars_clave_estadisticas <- train %>%
  select(Pobre, nmujeres, nmayor_edad, nafi_salud, 
         H_Head_Educ_level, maxEducLevel, 
         H_cotizando, pagada, hipoteca, arriendo, usufructo, sin_titulo,
         Clase, Nper, Lp, H_Head_mujer, H_afi_salud, H_Head_ocupado,
         nt_trab_semanal, ncotizando, nocupados)

# Función para calcular resumen de una variable
resumir_variable <- function(var_name, data) {
  x <- data[[var_name]]
  tibble(
    Variable = var_name,
    Media = mean(as.numeric(x), na.rm = TRUE),
    Mediana = median(as.numeric(x), na.rm = TRUE),
    Min = min(as.numeric(x), na.rm = TRUE),
    Max = max(as.numeric(x), na.rm = TRUE),
    Des.st = sd(as.numeric(x), na.rm = TRUE)
  )
}

# Generar lista de tibbles y unir en uno solo
resumen_final <- names(vars_clave_estadisticas) %>%
  map(~resumir_variable(.x, vars_clave_estadisticas)) %>%
  bind_rows()

# 3. (Opcional) Redondear valores
resumen_final <- resumen_final %>%
  mutate(across(where(is.numeric), ~round(., 2)))

 # Guardar 
write.table(resumen_final, 
            "../04_Views/TABLA3-RESUMEN-VARIABLES-ESTADISTICAS.txt", 
...
ggsave("../04_Views/GRAFICO2-PROPORCION_DE_CLASES.png", dpi = 150)
```
...
# Guardar gráfico de validación cruzada de modelo Elastic Net
png("../04_Views/GRÁFICO3-RESULTADOS DE VALIDACIÓN CRUZADA EN GENERAL.png", width = 1000, height = 600)
...
# Guardar gráfico
ggsave("../04_Views/GRAFICO4-MÉTRICAS_vs_UMBRAL_DECISION.png", plot = graf_metricas_EN_Gen, dpi = 150, width = 8, height = 6)
...
# Guardar
ggsave("../04_Views/GRAFICO5-CURVA_ROC_ELASTIC_NET_GEN.png", 
       plot = graf_roc_EN_gen, dpi = 150, width = 8, height = 6)
...
# Guardar como archivo de texto tabulado
write.table(counts_table_EN_gral, 
            file = "../04_Views/TABLA4-TABLA-CONTEOS-ELASTIC-NET-GEN.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
colnames(metrics_table_mod_1_EN_gral) <- c("Metrica", "Valor", "Descripcion")

# Imprimir tabla de métricas
print(metrics_table_mod_1_EN_gral)

# Guardar como archivo de texto tabulado
write.table(metrics_table_mod_1_EN_gral, 
            file = "../04_Views/TABLA5-MÉTRICAS-MODELO-ELASTIC-NET-GEN.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
ggsave("../04_Views/GRÁFICO6-IMPORTANCIA-VAR-ELASTIC-NET-GENERAL.png", 
...
png("../04_Views/GRÁFICO7-PROPORCIÓN DE CLASES DE POBRAZA IMPORTANTES EN ENTRENAMIENTO.png", width = 800, height = 600, res = 150)
...
# Guardar 
ggsave("../04_Views/GRAFICO8-PROPORCION-CLASES-ORIGINAL-VS-PONDERADA.png", 
...
# Guardar gráfico de validación cruzada de modelo Elastic Net
png("../04_Views/GRÁFICO9-RESULTADOS DE VALIDACIÓN CRUZADA EN.png", width = 1000, height = 600)
...
# Guardar gráfico
ggsave("../04_Views/GRAFICO10-MÉTRICAS_vs_UMBRAL_DECISION_EN.png", plot = graf_metricas_EN, dpi = 150, width = 8, height = 6)
...
graf_roc_EN <- ggplot() +
  ...
  geom_text(data = punto_umbral, 
            aes(x = Especificidad, y = Sensibilidad, 
                label = paste("Umbral =", round(umbral,4))),
            hjust = -0.2, vjust = -0.5, color = "red", size = 4) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray") +
  labs(
    title = "Curva ROC - Modelo Elastic Net",
  ...
  # Fondo blanco clásico y sin cuadrículas
...
# Guardar
ggsave("../04_Views/GRAFICO11-CURVA_ROC_ELASTIC_NET.png", 
       plot = graf_roc_EN, dpi = 150, width = 8, height = 6)
...
# Guardar como archivo de texto tabulado
write.table(counts_table_EN, 
            file = "../04_Views/TABLA6-TABLA-CONTEOS-ELASTIC-NET.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Renombrar las columnas del dataframe
colnames(metrics_table_mod_1_EN) <- c("Metrica", "Valor", "Descripcion")

# Imprimir tabla de métricas
print(metrics_table_mod_1_EN)

# Guardar como archivo de texto tabulado
write.table(metrics_table_mod_1_EN, 
            file = "../04_Views/TABLA7-MÉTRICAS-MODELO-ELASTIC-NET.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
colnames(comparacion_formateada) <- c("Métrica", "Modelo General", "Modelos Variables Import", "Diferencia", "Mejora porcental")

# Guardar como archivo de texto tabulado
write.table(comparacion_formateada, 
            file = "../04_Views/TABLA8-COMPARACIÓN-MÉTRICAS-MODELOS-ELASTIC-NET.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
comp_metricas_EN <- ggplot(comparacion_long, aes(x = Métrica, y = Valor, fill = Modelo)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8) +
...
#Guardar
ggsave("../04_Views/GRAFICO12-COMPARACIÓN_MÉTRICAS_ELASTIC_NET.png", 
       plot = comp_metricas_EN, dpi = 150, width = 8, height = 6)
...
#Guardar
ggsave("../04_Views/GRAFICO13-COMPARACIÓN_ROC_ELASTIC_NET.png", 
       plot = comp_ROC_EN, dpi = 150, width = 8, height = 6)

# 4. Comparación de hiperparámetros óptimos
hiperparametros_EN <- data.frame(
  Modelo = c("Modelo General", "Modelo Variables Importantes"),
...
print(hiperparametros_EN)

# Guardar como archivo de texto tabulado
write.table(hiperparametros_EN, 
            file = "../04_Views/TABLA9-COMPARACIÓN-HIPERPARÁMETROS-ELASTIC-NET.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
comp_matrices_EN <- ggplot(conf_mat_combined, aes(x = Actual, y = Predicted, fill = Count)) +
  geom_tile() +
  facet_wrap(~Modelo) +
...
#Guardar
ggsave("../04_Views/GRAFICO14-COMPARACIÓN_MATRIZ_CONFUSIÓN_ELASTIC_NET.png", 
       plot = comp_matrices_EN, dpi = 150, width = 8, height = 6)
```
...
# Guardar como archivo de texto tabulado
write.table(prediccion_test_EN, 
            file = "../04_Views/TABLA5-RESUMEN-PREDICCIÓN-ELASTIC-NET.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
png("../04_Views/GRAFICO15-DISTRIBUCION_PROBABILIDADES_EN.png", width = 800, height = 600)
...
# Iniciar entorno
png("../04_Views/GRAFICO16-PROPORCION_CLASES_PREDICHAS_EN.png", width = 800, height = 600)
...
# Guardar como archivo de texto tabulado
write.table(counts_table_train_logit, 
            file = "../04_Views/TABLA11-MATRIZ-CONFUSIÓN-LOGIT.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar como archivo de texto tabulado
write.table(metrics_table_mod_1_log, 
            file = "../04_Views/TABLA12-MÉTRICAS-MODELO1-LOGIT.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar
ggsave("../04_Views/GRAFICO5-CURVA_ROC_LOGIT.png", 
       plot = graf_roc_LOGIT, dpi = 150, width = 8, height = 6)
```
...
# Guardar como archivo de texto tabulado
write.table(counts_table_cv, 
            file = "../04_Views/TABLA13-MATRIZ-DE-CONFUSIÓN-LOGIT.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar como archivo de texto tabulado
write.table(metrics_table_mod_2_log, 
            file = "../04_Views/TABLA14-MÉTRICA-LOGIT2.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar como archivo de texto tabulado
write.table(comparar_df, 
            file = "../04_Views/TABLA15-COMPAR-MÉTRICA-LOGIT2.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar como archivo de texto tabulado
write.table(prediccion_test_logit, 
            file = "../04_Views/TABLA16-PREDICCIÓN-LOGIT2.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Iniciar entorno
png("../04_Views/GRAFICO18-HISTOGRAMA-LOGIT.png", width = 800, height = 600)
...
# Iniciar entorno
png("../04_Views/GRAFICO19-CLASES-PREDICHAS-LOGIT.png", width = 800, height = 600)
...
# Guardar como archivo de texto tabulado
write.table(comparacion_mod_log, 
            file = "../04_Views/TABLA17-COMPARACIÓN-MODELO-LOG.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Guardar como archivo de texto tabulado
write.table(randomforest, 
            file = "../04_Views/TABLA18-RESULTADOS-RANDOM-FOREST.txt", 
            sep = "\t", 
            row.names = FALSE, 
            quote = FALSE,
            fileEncoding = "UTF-8")
...
# Ver las primeras 10 filas en consola
head(resultados_prediccion_rf_cv, 10)
...
## 4.11 Comparar AUC de ambos modelos y F1 del segundo modelo
...
# Ver los resultados del entrenamiento cruzado
cv_rf$results
...
```{r}
cv_rf_weighted_quad$results  # muestra ROC, Sensibilidad, Especificidad, etc.
```
...
png("../04_Views/GRAFICO20-CURVA-ROC-RF-WEIGHTED-TRAIN.png", width = 800, height = 600)
plot(roc_rf, col = "darkgreen", main = "Curva ROC - Random Forest Weighted (Train)")
dev.off()
```
```{r}
# Mostrar el resultado del AUC
print(paste("AUC:", round(auc_rf, 4)))
```
...
```{r}
# Predecir probabilidades sobre el mismo train_nb
pred_prob_train <- predict(nb_model, newdata = train_nb, type = "prob")

# Clasificar usando umbral 0.3
pred_clase_train <- ifelse(pred_prob_train[, "Yes"] >= 0.3, "Yes", "No")

# Convertir a factor con niveles correctos
pred_clase_train <- factor(pred_clase_train, levels = c("No", "Yes"))
real_train <- factor(train_nb$Pobre, levels = c("No", "Yes"))

# Matriz de confusión
confusionMatrix(pred_clase_train, real_train, positive = "Yes")

```


## 5.4 Entrenamiento CV
...
# Ver tabla resumen
tabla_pred_nb <- table(predicciones_kaggle_nb$Pobre)

tabla_pred_nb_df <- as.data.frame(tabla_pred_nb)
colnames(tabla_pred_nb_df) <- c("Clase", "Frecuencia")

# 3. Guardar como .txt para Overleaf
write.table(tabla_pred_nb_df,
            file = "../04_Views/TABLA20-PREDICCIONES-NB.txt",
            sep = "\t",            # Tabulación para pgfplotstable
            row.names = FALSE,
            quote = FALSE)
```

## 5.6 Template Naive Bayes
...
tabla_xgb <- table(resultados_prediccion_xgboost_2$Pobre)

tabla_xgb_df <- as.data.frame(tabla_xgb)
colnames(tabla_xgb_df) <- c("Clase", "Frecuencia")

write.table(tabla_xgb_df,
            file = "../04_Views/TABLA21-PREDICCIONES-XGBOOST.txt",
            sep = "\t",            # Tabulación para pgfplotstable
            row.names = FALSE,
            quote = FALSE)
...
write.csv(resultados_prediccion_xgboost_2,
          "C:/../05_Predictions/06-04-25_equipo4_xgboost_cv.csv", row.names = FALSE, 
          quote = FALSE)
```

##6.6 Revision de importancia de variables

```{r}
library(caret)
png("../04_Views/GRAFICO21-IMPORTANCIA-VARIABLES-XGBOOST.png", width = 1000, height = 800)
plot(varImp(xgb_model), top = 20, 
     main = "Top 20 variables más importantes - XGBoost")
dev.off()
```

##6.7 Metricas de desempeño en validación cruzada

```{r}
library(yardstick)
library(dplyr)

# Acceder a las predicciones de validación cruzada
predicciones_cv <- xgb_model$pred

# Elegir la mejor combinación de hiperparámetros
mejores_param <- xgb_model$bestTune
predicciones_filtradas <- predicciones_cv %>%
  filter(nrounds == mejores_param$nrounds,
         max_depth == mejores_param$max_depth,
         eta == mejores_param$eta,
         gamma == mejores_param$gamma,
         min_child_weight == mejores_param$min_child_weight,
         colsample_bytree == mejores_param$colsample_bytree,
         subsample == mejores_param$subsample)

# Calcular F1, precisión, sensibilidad (recall)
f1_resultado <- f_meas(predicciones_filtradas, truth = obs, estimate = pred, event_level = "second")
precision_resultado <- precision(predicciones_filtradas, truth = obs, estimate = pred, event_level = "second")
recall_resultado <- recall(predicciones_filtradas, truth = obs, estimate = pred, event_level = "second")

# AUC
auc_resultado <- roc(response = predicciones_filtradas$obs,
                     predictor = predicciones_filtradas$Yes,
                     levels = c("No", "Yes"),
                     direction = "<") 

# Accuracy
accuracy_resultado <- accuracy(predicciones_filtradas,
                               truth = obs,
                               estimate = pred)
...
##6.8 Inclusión de interacciones en el modelo

```{r}
#Creacion bases
train_inter <- train_factors
test_inter <- test_factors

```

##6.9 Creación de interacciones de variables para entrenamiento

```{r}
# Crear dummy: 1 si maxEducLevel es Universitaria
train_inter$educ_univ <- ifelse(train_inter$maxEducLevel == "Universitaria", 1, 0)

# Crear interacción: educación universitaria x número de personas
train_inter$educ_x_nper <- train_inter$educ_univ * train_inter$Nper
```

##6.10 Creación de interacciones de variables para test

```{r}
# Crear dummy: 1 si tiene educación universitaria, 0 en caso contrario
test_inter$educ_univ <- ifelse(test_inter$maxEducLevel == "Universitaria", 1, 0)

# Crear variable de interacción: educación universitaria x número de personas
test_inter$educ_x_nper <- test_inter$educ_univ * test_inter$Nper
```

##6.11 Entrenamiento CV con interacciones

```{r}
# Entrenar el modelo XGBoost con fórmula directa
set.seed(123)
xgb_model_inter <- train(
  Pobre ~ .,                          
  data = train_inter,               
  method = "xgbTree",
  trControl = ctrl,
...
print(xgb_model_inter)
```
##6.12 Predicción modelo interacciones

```{r}
# Obtener probabilidades
pred_prob_xgboost_interaccion <- predict(xgb_model_inter, newdata = test_inter, type = "prob")
pred_clase_xgboost_interaccion <- ifelse(pred_prob_xgboost_interaccion[, "Yes"] >= 0.5, 1, 0)
resultados_prediccion_xgboost_inter <- data.frame(id = test$id, Pobre = pred_clase_xgboost_interaccion)

tabla_xgb_inter <- table(resultados_prediccion_xgboost_inter$Pobre)

# Convertir la tabla en un data frame
tabla_xgb_inter_df <- as.data.frame(tabla_xgb_inter)

# Renombrar columnas
colnames(tabla_xgb_inter_df) <- c("Clase", "Frecuencia")

write.table(tabla_xgb_inter_df,
            file = "../04_Views/TABLA22-PREDICCION-XGBOOST-INTER.txt",
            sep = "\t",
            row.names = FALSE,
            quote = FALSE)
```
##6.13 Metricas de desempeño con interacciones
...
predicciones_cv_inter <- xgb_model_inter$pred

# Elegir la mejor combinación de hiperparámetros
mejores_param_inter <- xgb_model_inter$bestTune
predicciones_filtradas_inter <- predicciones_cv_inter %>%
  filter(nrounds == mejores_param_inter$nrounds,
         max_depth == mejores_param_inter$max_depth,
         eta == mejores_param_inter$eta,
         gamma == mejores_param_inter$gamma,
         min_child_weight == mejores_param_inter$min_child_weight,
         colsample_bytree == mejores_param_inter$colsample_bytree,
         subsample == mejores_param_inter$subsample)

# Calcular F1, precisión, sensibilidad (recall)
f1_inter <- f_meas(predicciones_filtradas_inter, truth = obs, estimate = pred, event_level = "second")
precision_inter <- precision(predicciones_filtradas_inter, truth = obs, estimate = pred, event_level = "second")
recall_inter <- recall(predicciones_filtradas_inter, truth = obs, estimate = pred, event_level = "second")

#Calcular Accuracy
accuracy_inter <- accuracy(predicciones_filtradas_inter, truth = obs, estimate = pred)

# Calcular AUC
auc_inter <- roc_auc(predicciones_filtradas_inter, truth = obs, Yes, event_level = "second")
...
##6.14 Template XGBoost con interacciones
```{r}
## Template_XGB
write.csv(resultados_prediccion_xgboost_interacciones,
          "C:/../05_Predictions/06-04-25_equipo4_xgboost_interacciones.csv", row.names = FALSE, 
          quote = FALSE)
...
## 7.2 Predicción
```{r}
# Predecir en test_factors
predictTest_cart <- test_factors %>%
  mutate(
    prob_pobre_cart = predict(
      arbol, 
      newdata = test_factors[, setdiff(names(test_factors), "id")], 
      type = "prob"
    )[,"Yes"],

    pred_pobre_cat_cart = ifelse(prob_pobre_cart > 0.5, "Yes", "No"),
    pobre = ifelse(pred_pobre_cat_cart == "Yes", 1, 0)
  )

# Guardar resultados
resultados_prediccion_cart <- predictTest_cart %>%
  select(id, pobre)
```

# 7.3 Kaggle
```{r}
write.csv(resultados_prediccion_cart, 
          "C:/MECA/2025/BIG DATA Y MACHINE LEARNING- Ignasio Sarmiento/Taller2/T2_BDML/05_Predictions/100425sample_equipo4_cart.csv", 
          row.names = FALSE)

```


## 7.4 Entrenamiento del modelo- balanceado
```{r}
library(caret)
set.seed(123)
...
modelo_cart_cv <- train(
  Pobre ~ Nper + nmujeres + ncotizando + maxEducLevel + nt_trab_semanal + 
    H_cotizando + nmayor_edad,
  data = train_factors,
  method = "rpart",
  trControl = control,
  metric = "ROC",               # AUC como métrica principal
  tuneLength = 10               
)

# Ver resultados
modelo_cart_cv
png("../04_Views/GRAFICO22-VALIDACION-CRUZADA-CART.png", width = 800, height = 600)
plot(modelo_cart_cv)
dev.off()
...
```{r}
# Asignar umbral de clasificación
umbral_gral_cart <- 0.5

# Probabilidades en entrenamiento
prob_train_cart <- predict(arbol_cart, newdata = train_factors, type = "prob")[, "Yes"]

# Clasificación binaria usando umbral general
pred_train_cart <- ifelse(prob_train_cart > umbral_gral, "Yes", "No")

# Matriz de confusión
confusionMatrix(factor(pred_train_cart), factor(train_factors$Pobre), positive = "Yes")

# Métricas
library(MLmetrics)

precision <- Precision(y_pred = pred_train_cart, y_true = train_factors$Pobre, positive = "Yes")
recall <- Recall(y_pred = pred_train_cart, y_true = train_factors$Pobre, positive = "Yes")
f1 <- F1_Score(y_pred = pred_train_cart, y_true = train_factors$Pobre, positive = "Yes")
...
# Curva ROC y AUC
library(pROC)
roc_cart <- roc(response = train_factors$Pobre, predictor = prob_train_cart, levels = c("No", "Yes"))
auc_cart <- auc(roc_cart)
cat(paste0("AUC: ", round(auc_cart, 4), "\n"))

# Gráfico ROC
png("../04_Views/GRAFICO23-CURVA-ROC-CART.png", width = 800, height = 600)

plot(roc_cart, col = "blue", main = "Curva ROC - CART")

dev.off()

```

## 7.6 Predicciones para Kaggle
```{r}
# Probabilidades en test
prob_test_cart <- predict(
  arbol_cart, 
  newdata = test_factors[, setdiff(names(test_factors), "id")], 
  type = "prob"
)[,"Yes"]
...
pred_test_cart <- ifelse(prob_test_cart > umbral_gral_cart, 1, 0)

# Construcción del dataframe final para Kaggle
resultados_prediccion_cart <- data.frame(
  id = test_factors$id,
  pobre = pred_test_cart
)
...
# Guardar Kaggle
write.csv(
  resultados_prediccion_cart,
  "C:/MECA/2025/BIG DATA Y MACHINE LEARNING- Ignasio Sarmiento/Taller2/T2_BDML/05_Predictions/10-04-24cartcv.csv",
  row.names = FALSE,
  quote = FALSE
)
...
# Guardar
ggsave("../04_Views/GRÁFICO24-IMPORTANCIA DE VARIABLES REGRESIÓN LINEAL.png",
