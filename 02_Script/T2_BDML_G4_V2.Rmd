...
##2.3 Entrenar el modelo Elastic Net con Caret
```{r}
#Modelo Elastic Net
...
mod_1_EN <- train(Pobre~., #Modelo 1 Elastic Net
    ...
    weights = wts,  # Inclusion de penderacion
...
##2.5 Predicción modelo Elastic Net
```{r}
...
# Obtener predicciones en los datos de entrenamiento de acuerdo con el umbral
...
##2.6 Crear la curva ROC para prediccion train set
```{r}
# Crear objeto ROC

roc_obj <- roc(response = train_factors$Pobre, 
               predictor = prob_train)
# Calcular AUC
auc_value <- auc(roc_obj)

# Calcular punto específico en la curva ROC para el umbral 0.3
conf_matrix <- confusionMatrix(predicciones_train, train_factors$Pobre)
...
##2.8 Metricas Modelo Elastic Net
```{r}
# Calcular métricas
...
##2.9 Explorar la importancia de las variables
```{r}
...
##2.10 Prediccion con el Test Set Elastic Net
```{r}
...
##2.11 Gráficos de la predicción
```{r}
...
predicciones_train_cv <- predict(mod_2_logit, newdata = train_factors)

# Matriz de confusión del modelo con validación cruzada
B_cv <- confusionMatrix(predicciones_train_cv, train_factors$Pobre)
B_cv <- B_cv$table
...
```
##3.10 Metricas Modelo de Entrenamieno Logit 
```{r}
#Métricas derivadas:
# Exactitud (Accuracy): (TP + TN) / (TP + TN + FP + FN)
# Proporción de predicciones correctas.
...
##3.11 Explorar la importancia de las variables
```{r}
...
##3.12 Comparacion entre los modelos 1 y 2
```{r}
...
##3.13 Predición con el Test Set/Clasificacion 
```{r}
#Prediccion el Test Set
predictTest_logit <- test_factors %>% 
  mutate(
    ...
    pred_pobre_cat_logit = ifelse(prob_pobre_logit > 0.4, "Yes", "No"), #Cambio del umbral de referencia
...
##3.14 Gráficos de la predicción
```{r}
# Distribucion de probabilidades predichas
...
##3.15 Comparacion entre entrenamiento y prediccion
...
##3.16 Template_Logit
```{r}
write.csv(resultados_prediccion_logit, "/Users/jorgeviafara/Library/CloudStorage/OneDrive-Personal/MAESTRIA EN ECONOMIA/01 MATERIAS MECA/4107 BIG DATA/T2_BDML_personal/01042025sample_equipo4_Logit.csv", row.names = FALSE)
```
