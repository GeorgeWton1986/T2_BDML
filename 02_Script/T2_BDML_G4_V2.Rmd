...
imp_df <- data.frame(
  Variable = rownames(importancia_vars$importance),
...
##2.6 Predicción modelo Elastic Net
```{r}
...
##2.7 Optimización de umbral para maximizar F1 Score
```{r}
# Calculamos la curva PR para 100 valores de umbral
prec_recall <- data.frame(coords(roc(response = train_factors$Pobre, predictor = prob_train), 
                                seq(0, 1, length=100), 
...
##2.8 Crear la curva ROC para prediccion train set Elastic Net
```{r}
# Crear objeto ROC
...
##2.10 Metricas Modelo Elastic Net
```{r}
# Calcular métricas
...
##2.11 Prediccion con el Test Set Elastic Net
```{r}
...
##2.12 Gráficos de la predicción
```{r}
