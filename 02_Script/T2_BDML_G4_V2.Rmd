...
mod_1_EN_gral <- train(Pobre~., #Modelo 1 Elastic Net
    ...
    weights = wts,  # Inclusion de penderacion de clases
...
##2.5 Estimacion de coeficientes por Elastic Net
```{r}
...
##2.6 Seleccionar variables importantes y ponderacion
```{r}
...
##2.7 Graficos proporcicion de la muestra variables importantes
```{r}
# Calcular proporción de clases (No pobre vs Pobre)
...
# Crear vector de pesos
wts_import <- ifelse(train_importantes$Pobre == "Yes", pos_weight, 1)

# Calcular distribución ponderada
tabla_ponderada_import <- c(
  sum(wts[train_importantes$Pobre == "No"]),  # Suma de pesos para "No pobre"
  sum(wts[train_importantes$Pobre == "Yes"])  # Suma de pesos para "Pobre"
)
...
# Crear gráfico de barras agrupadas
ggplot(datos_plot_import, aes(x = Clase, y = Proporcion, fill = Tipo)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8) +
  geom_text(aes(label = paste0(round(Proporcion * 100, 1), "%")), 
            position = position_dodge(width = 0.9),
            vjust = -0.5, size = 3.5) +
...
##2.8 Entrenar el modelo Elastic Net con Caret variables importantes
```{r}
...
##2.9 Predicción modelo Elastic Net
```{r}
...
##2.10 Optimización de umbral para maximizar F1 Score
```{r}
...
##2.11 Crear la curva ROC para prediccion train set Elastic Net
```{r}
# Crear objeto ROC
...
##2.13 Metricas Modelo Elastic Net
```{r}
# Calcular métricas
...
##2.14 Prediccion con el Test Set Elastic Net
```{r}
...
##2.15 Gráficos de la predicción
```{r}
...
##3.18 Template_Logit
```{r}
write.csv(resultados_prediccion_logit, "/Users/jorgeviafara/Library/CloudStorage/OneDrive-Personal/MAESTRIA EN ECONOMIA/01 MATERIAS MECA/4107 BIG DATA/T2_BDML_personal/08042025sample_equipo4_Logit.csv", row.names = FALSE)
```
...
```{r}
## Template_RF
write.csv(resultados_prediccion_rf, "C:/MECA/2025/BIG DATA Y MACHINE LEARNING- Ignasio Sarmiento/Taller2/T2_BDML/Predictions/080425sample_equipo4_rf.csv", row.names = FALSE)
```
...
```{r}
## Template_RF
write.csv(resultados_prediccion_rf_cv, "C:/MECA/2025/BIG DATA Y MACHINE LEARNING- Ignasio Sarmiento/Taller2/T2_BDML/Predictions/080425sample_equipo4_rf_cv.csv", row.names = FALSE)
```
...
##7.1. Detección variables varianza cero
...
##7.5.Template Naive Bayes
```{r}
## Template_RF
write.csv(predicciones_kaggle_nb, "C:/Users/njaco/OneDrive/Documentos/Mestría en Economía Aplicada/Semestre 3/Big Data y Machine Learning/Repositorios/T2_BDML/05_Predictions/sample_equipo4_naivebayes.csv", row.names = FALSE)
```
...
# 6. Entrenamiento algoritmo CART
