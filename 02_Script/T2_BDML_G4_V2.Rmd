...
##5.6. ROC y AUC con Naive Bayes base entrenamiento 
```{r}
# Predicciones sobre el train 
...
# Curva ROC y AUC
roc_train <- roc(response = train_nb$Pobre, predictor = pred_prob_train[, "Yes"])
plot(roc_train, col = "darkgreen", main = "Curva ROC - Train (Naive Bayes)")
auc(roc_train)
...
# Evaluar F1 Score para varios umbrales
umbrales <- seq(0.1, 0.9, by = 0.05)
f1_scores <- sapply(umbrales, function(thresh) {
  pred <- ifelse(pred_prob_nb[, "Yes"] >= thresh, 1, 0)
  F1_Score(y_true = as.numeric(test$Pobre == "Yes"), y_pred = pred)
})

# Gráfico de F1 Score según umbral
plot(umbrales, f1_scores, type = "b", col = "darkred", 
     main = "F1 Score según umbral", xlab = "Umbral", ylab = "F1 Score")
```
